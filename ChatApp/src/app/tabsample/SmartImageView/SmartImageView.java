...
public class SmartImageView extends ImageView {
	...
	protected void onDraw(Canvas canvas) {
		...
		if (dr != null) {
			canvas.drawColor(Color.parseColor("#e9eae1"));
			Bitmap output = ((BitmapDrawable) dr).getBitmap();
			Paint paint = new Paint();
			canvas.drawBitmap(getclip(output, canvas), 0, 0, paint);
		}
	}

	public Bitmap getclip(Bitmap bitmap, Canvas canvas_) {
		...
		final Rect rect = new Rect(0, 0, canvas_.getWidth(),
				canvas_.getHeight());

		paint.setAntiAlias(true);

		canvas.drawCircle(canvas_.getWidth() / 2, canvas_.getHeight() / 2,
				canvas_.getWidth() / 2, paint);
